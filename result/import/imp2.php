...
<center>
<form enctype="multipart/form-data" method="post" role="form" action="imp2.php" class="import">
    <div class="form-group">
        <label for="exampleInputFile">Upload 2 Semester File</label>
...
if(isset($_POST["submit"]))
{
		  ...
		  $english_internal = $filesop[5];
		    $ds_theory = $filesop[6];
		  $ds_internal = $filesop[7];
		    $dbms_theory = $filesop[8];
		  $dbms_internal = $filesop[9];
		  $nsm_theory = $filesop[10];
		  $nsm_internal = $filesop[11];
		    $ds_lab_theory = $filesop[12];
		  $ds_lab_internal = $filesop[13];
		    $dbms_lab_theory = $filesop[14];
		  $dbms_lab_internal = $filesop[15];
		  $ichr_theory = $filesop[16];
		  $ichr_internal = $filesop[17];
		  	  $ccec = $filesop[18];
		  ...
		  $gpa = $filesop[21];
		    $grade = $filesop[22];
		  $result = $filesop[23];
         $sql = "insert into 2sem(regno,name,il_theory,il_internal,english_theory,english_internal,ds_theory,ds_internal,dbms_theory,dbms_internal,nsm_theory,nsm_internal,ds_lab_theory,ds_lab_internal,dbms_lab_theory,dbms_lab_internal,ichr_theory,ichr_internal,ccec,total_theory,total_internal,gpa,grade,result) values ('$regno','$name','$il_theory','$il_internal','$english_theory','$english_internal','$ds_theory','$ds_internal','$dbms_theory','$dbms_internal','$nsm_theory','$nsm_internal','$ds_lab_theory','$ds_lab_internal','$dbms_lab_theory','$dbms_lab_internal','$ichr_theory','$ichr_internal','$ccec','$total_theory','$total_internal','$gpa','$grade','$result')";
