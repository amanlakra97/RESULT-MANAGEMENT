...
<?php
$sql="select total_theory,count(regno) as distinction from 1sem where total_theory between 392 and 520";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select total_theory,count(regno) as first from 1sem where total_theory between 319 and 391";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select total_theory,count(regno) as second from 1sem where total_theory between 273 and 318";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select total_theory,count(regno) as pass from 1sem where total_theory between 204 and 272";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select total_theory,count(regno) as minimum from 1sem where total_theory between 1 and 203";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
...
<div class="btn9">
<form action="1totaltheory.php" method="post">
