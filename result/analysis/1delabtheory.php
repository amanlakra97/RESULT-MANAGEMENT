...
<?php
$sql="select de_lab_theory,count(regno) as distinction from 1sem where de_lab_theory between 26 and 35";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select de_lab_theory,count(regno) as first from 1sem where de_lab_theory between 21 and 25";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select de_lab_theory,count(regno) as second from 1sem where de_lab_theory between 17 and 20";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select de_lab_theory,count(regno) as pass from 1sem where de_lab_theory between 12 and 16";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select de_lab_theory,count(regno) as minimum from 1sem where de_lab_theory between 1 and 11";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
$dataPoints = array(
	...
	array("label"=> "1st Class", "y"=> $d2),
	array("label"=> "2nd Class", "y"=> $d3),
	array("label"=> "Pass", "y"=> $d4),
	array("label"=> "Fail", "y"=> $d5),
...
<div class="btn9">
<form action="1delabtheory.php" method="post">
