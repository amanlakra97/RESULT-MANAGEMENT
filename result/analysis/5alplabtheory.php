...
<?php
$sql="select alp_lab_theory,count(regno) as distinction from 5sem where alp_lab_theory between 26 and 35";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select alp_lab_theory,count(regno) as first from 5sem where alp_lab_theory between 21 and 25";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select alp_lab_theory,count(regno) as second from 5sem where alp_lab_theory between 17 and 20";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select alp_lab_theory,count(regno) as pass from 5sem where alp_lab_theory between 12 and 16";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select alp_lab_theory,count(regno) as minimum from 5sem where alp_lab_theory between 1 and 11";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	title:{
		text: "MICROPROCESSOR AND ASSEMBLY LANGUAGE LAB"
	},
	data: [{
