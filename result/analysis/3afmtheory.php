...
<?php
$sql="select afm_theory,count(regno) as distinction from 3sem where afm_theory between 53 and 70";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select afm_theory,count(regno) as first from 3sem where afm_theory between 42 and 52";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select afm_theory,count(regno) as second from 3sem where afm_theory between 35 and 41";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select afm_theory,count(regno) as pass from 3sem where afm_theory between 25 and 34";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select afm_theory,count(regno) as minimum from 3sem where afm_theory between 1 and 24";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	title:{
		text: "FINANCIAL ACCOUNTING AND MANAGEMENT"
	},
	data: [{
...
<div class="btn9">
<form action="3afmtheory.php" method="post">
