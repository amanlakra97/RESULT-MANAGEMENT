...
<?php
$sql="select sp_theory,count(regno) as distinction from 6sem where sp_theory between 75 and 100";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select sp_theory,count(regno) as first from 6sem where sp_theory between 60 and 74";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select sp_theory,count(regno) as second from 6sem where sp_theory between 50 and 59";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select sp_theory,count(regno) as pass from 6sem where sp_theory between 35 and 50";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select sp_theory,count(regno) as minimum from 6sem where sp_theory between 1 and 34";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
...
<div class="btn9">
<form action="6sptheory.php" method="post">
