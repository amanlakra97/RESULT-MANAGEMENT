...
<?php
$sql="select dm_theory,count(regno) as distinction from 1sem where dm_theory between 75 and 100";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select dm_theory,count(regno) as first from 1sem where dm_theory between 60 and 74";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select dm_theory,count(regno) as second from 1sem where dm_theory between 50 and 59";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select dm_theory,count(regno) as pass from 1sem where dm_theory between 35 and 50";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select dm_theory,count(regno) as minimum from 1sem where dm_theory between 1 and 34";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
...
<div class="btn9">
<form action="1dmtheory.php" method="post">
