...
<?php
$sql="select nsm_theory,count(regno) as distinction from 2sem where nsm_theory between 75 and 100";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d1=$rs['distinction'];
	}
$sql="select nsm_theory,count(regno) as first from 2sem where nsm_theory between 60 and 74";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d2=$rs['first'];
	}
$sql="select nsm_theory,count(regno) as second from 2sem where nsm_theory between 50 and 59";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d3=$rs['second'];
	}
$sql="select nsm_theory,count(regno) as pass from 2sem where dm_theory between 35 and 50";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d4=$rs['pass'];
	}
$sql="select nsm_theory,count(regno) as minimum from 2sem where nsm_theory between 1 and 34";
$query=mysqli_query($con,$sql);
while($rs=mysqli_fetch_assoc($query)){
	$d5=$rs['minimum'];
	}
...
var chart = new CanvasJS.Chart("chartContainer", {
	...
	title:{
		text: "NUMERICAL AND STATISTICAL METHODS"
	},
	data: [{
...
<div class="btn9">
<form action="2nsmtheory.php" method="post">
